...
!	or %l1, %l0, %l0
	...
	!basic case: 2.0 + 3.0 = 5.0
	set A, %l0
	ld [%l0], %f0
	ld [%l0+4], %f1
	ld [%l0+8], %f2
	ld [%l0+12], %f3	! f0-f3 = 2.0 (quad)
	set B, %l1
	ld [%l1], %f4
	ld [%l1+4], %f5
	ld [%l1+8], %f6
	ld [%l1+12], %f7	! f4-f7 = 3.0 (quad)
	faddq %f0, %f4, %f8	! f8-f11 = 5.0 (quad)
	rd %psr, %o1		! Expected o1=0x10E0

	!invalid case: (+inf) + (-inf) -- traps into HW_trap_0x08, which increments
	!g2 and resumes via rett (see trap table below)
	set C, %l2
	ld [%l2], %f12
	ld [%l2+4], %f13
	ld [%l2+8], %f14
	ld [%l2+12], %f15	! f12-f15 = +inf (quad)
	set D, %l3
	ld [%l3], %f16
	ld [%l3+4], %f17
	ld [%l3+8], %f18
	ld [%l3+12], %f19	! f16-f19 = -inf (quad)
	faddq %f12, %f16, %f20	! invalid: (+inf)+(-inf) -- g2 -> 1
...
not_reached:
	...
	.align 8
mFSR: .word 0
A: .word 0x40000000, 0x00000000, 0x00000000, 0x00000000
B: .word 0x40008000, 0x00000000, 0x00000000, 0x00000000
C: .word 0x7fff0000, 0x00000000, 0x00000000, 0x00000000
D: .word 0xffff0000, 0x00000000, 0x00000000, 0x00000000
