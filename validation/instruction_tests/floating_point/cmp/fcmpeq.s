...
!	or %l1, %l0, %l0
	...
	!instruction = FCMPEq
	!2.0 < 3.0 -> fcc=1 (same as FCMPq for ordered operands)
	set A, %l0
	ld [%l0], %f0
	ld [%l0+4], %f1
	ld [%l0+8], %f2
	...
	fcmpeq %f0, %f4	! 2.0 < 3.0 -> fcc=1
	set fsrsave0, %o4
	st %fsr, [%o4]
	ld [%o4], %o0		! o0: fsr, low bits carry the fcc field
	rd %psr, %o1		! Expected o1=0x10E0

	!unlike FCMPq, FCMPEq traps on a *quiet* NaN operand too -- g2 -> 1
	set C, %l2
	ld [%l2], %f12
	ld [%l2+4], %f13
	ld [%l2+8], %f14
	ld [%l2+12], %f15	! f12-f15 = quiet NaN (quad)
	fcmpeq %f0, %f12	! invalid: 2.0 vs qNaN, FCMPE traps on any NaN
