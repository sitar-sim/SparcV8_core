! Test: Ticc trap-number computation wraps modulo 128 when
! r[rs1]+r[rs2] exceeds 0x7F (SparcCore::execute_Ticc:
! "ticc_trap_type = readBits(trap_number, 6, 0)" -- Ref Appendix B.29 /
! Appendix C.9). Existing traps/ tests (ta.s etc.) only use small fixed
! trap numbers that stay within [0,0x7F] and never exercise this masking.
!
! %l0=200 (0xC8), %l1=200 (0xC8): r[rs1]+r[rs2] = 400 (0x190), which
! overflows even the full byte range, let alone 7 bits. 400 mod 128 = 16
! (0x10), so the resulting software trap number is 0x80|0x10 = 0x90 --
! not the raw sum, and not a naive 8-bit truncation of it (0x190 & 0xFF
! would be 0x90 too, coincidentally, but 0x190 & 0x7F is the actual
! mechanism being tested; see division_by_zero-style larger sums below in
! comments for a case where the two would differ if this were buggy).
!
! This test's own deliberate trap is what ends the test (the trap number
! it's routed to, 0x90, is not one of this file's own custom-purpose
! handlers, so it falls through to the generic "mov <tt>,%g1; restore;
! ta 0" trap-table entry, which halts immediately with g1=0x90 since
! traps are disabled (ET=0) inside that handler -- see
! validation/README.md's "Test format and pass/fail convention").
!
...
_start:
	...
	!======================================
	! Perform instruction test
	!
	! instruction = TA (register-register form)

	mov 200, %l0
	mov 200, %l1
	ta %l0 + %l1		! r[rs1]+r[rs2] = 400 -- trap number wraps
				! to 0x90 mod 128 (see header comment).
				! Expected g1=0x90.

	!======================================

	!control should NOT reach here -- the Ticc above always traps
not_reached:
	set 0xDEAD, %g1
	ta 0
	nop
