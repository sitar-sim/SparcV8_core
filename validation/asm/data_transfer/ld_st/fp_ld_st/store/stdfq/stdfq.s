! Instruction being verified : stdfq
!
...
!
! This implementation hardwires implementation_has_no_floating_point_queue=1
! (see model/cpp_common_code/SparcCore.cpp) -- there is no floating-point
! deferred-trap queue at all, so STDFQ must always trap fp_exception with
! FSR.ftt=sequence_error (Ref Appendix B.31 / Appendix C.9), regardless of
! FSR.qne, and must not perform the store. Mirrors the sibling coprocessor
! test data_transfer/ld_st/cp_ld_st/stdcq/stdcq.s.
...
_start:
	...
	! Enable FP unit (PSR.EF, bit 12)
	rd %psr, %l0
	set 0x1000, %l1
	or %l0, %l1, %l0
	wr %l0, %psr

	!store the first number and the adjacent one
	set data_A, %o0		! o0 = 0x70
	std %fq, [%o0]		! should trap: fp_exception / sequence_error.
				! mem[0x70]/mem[0x74] must stay unchanged (0x0A/0x0B),
				! since the store never executes.
