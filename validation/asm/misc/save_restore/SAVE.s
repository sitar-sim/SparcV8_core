...
_start:
	...
	!----------------------------------------------------------
	! check if addition of operands upon a save works okay
	
				! CWP=0

	set 0xBAD, %o0		! store a value in o0, which should 
	set 0x2, %l0		! get overwritten by save
	save %l0, 0x3, %i0	! i0 of new window = o0 of previous 
				! window, is overwritten.

	rd  %psr, %g3
	and %g3, 0x01F, %g3	! store CWP in g3. Expected g3=0x00000007
	restore;
	mov %o0, %g4		! Expected g4=0x5
	!----------------------------------------------------------
	
	!Check if saving to an invalid window causes a trap
	...
	save	!cwp=6
	rd  %psr, %g5
	and %g5, 0x01F, %g5	! store CWP in g5. Expected g5=0x00000006

	set 0x1234, %g6
	set 0x2, %l0
	save %l0, 0x3, %g6	! this should cause a trap 
				! because window 5 is invalid. 
				! g6 should be unmodified
				! Expected g6=0x00001234
				! and
				! Expected g2=0x00000001 (set inside windows_ovf trap handler)
	rd  %psr, %g7
	and %g7, 0x01F, %g7	! store CWP in g7. Expected g7=0x00000006 (unchanged)
	!----------------------------------------------------------
...
HW_trap_0x05: inc %g2; rett %r18; nop; nop; 	! WINDOW OVERFLOW TRAP HANDLER
