...
_start:
	...
	!
	! instruction = RESTORE
	! note : two operands are added and sum is placed in r[rd],
	! and CWP is updated only if the instruction does not trap.

	mov 0x0, %g2

	! First, mark all windows as valid
	wr %g0, %wim
	nop
	nop
	nop
	...
	!----------------------------------------------------------
	! check if addition of operands upon a restore works okay
	
				! CWP=0

	set 0xBAD, %o0		! store a value in o0, which should 
	set 0x2, %l0		! get overwritten by save
	restore %l0, 0x3, %o0	! o0 of new window = i0 of previous window


	rd  %psr, %g3
	and %g3, 0x01F, %g3	! store CWP in g3. Expected g3=0x00000001
	save;
	mov %i0, %g4		! Expected g4=0x5
	!----------------------------------------------------------
	
	!Check if restoring to an invalid window causes a trap
	!mark windows 5 as invalid
	wr %g0, 0x20, %wim
	nop
	nop
	nop
	...
	rd  %psr, %g5
	and %g5, 0x01F, %g5	! store CWP in g5. Expected g5=0x00000004

	set 0x1234, %g6
	set 0x2, %l0
	restore %l0, 0x3, %g6	! this should cause a trap 
				! because window 5 is invalid. 
				! g6 should be unmodified
				! Expected g6=0x00001234
				! and
				! Expected g2=0x00000001 (set inside windows_undf trap handler)
	rd  %psr, %g7
	and %g7, 0x01F, %g7	! store CWP in g7. Expected g7=0x00000004 (unchanged)
	!----------------------------------------------------------
...
HW_trap_0x06: inc %g2; rett %r18; nop; nop; 	! WINDOW UNDERFLOWTRAP HANDLER
