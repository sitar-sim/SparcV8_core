! Test: illegal_instruction trap via a genuinely reserved/unassigned
! opcode encoding, as distinct from the already-tested UNIMP instruction
! (see misc/stbar_unimp_nop_sethi/).
!
! UNIMP (op=0,op2=0) is itself a *defined* SPARC V8 instruction whose whole
! purpose is to always trap illegal_instruction -- it exercises the
! "(op()==0 and op2()==0)" branch in SparcCore::checkInstructionException().
! This test instead hand-encodes a format-3 instruction word with a
! genuinely reserved op3 field (op=2, op3=0x09 -- see the "op3 Encoding"
! table in Decoder.cpp, where 0x09/0x0D/0x19/0x1D/0x2C-0x2F/0x3E/0x3F are
! all unassigned), exercising the other branch:
! "((op()==2 or op()==3) and op==unassigned)" (Ref Appendix B.2 / SparcCore.cpp).
!
! Encoding of the reserved instruction word (0x80480000):
!   op=2 (10b), rd=%g0, op3=0x09 (001001b, reserved), rs1=%g0, i=0, rs2=%g0
!
...
_start:
	...
	!
	! g2 is incremented in the illegal_instruction trap handler.
	! Expected g2=1

	mov 0x0, %g2

	.word 0x80480000	! reserved/unassigned format-3 encoding --
				! should cause an illegal_instruction trap.
...
HW_trap_0x02: inc %g2; rett %r18; nop; nop !Illegal instruction trap handler
