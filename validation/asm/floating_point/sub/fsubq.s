...
!	or %l1, %l0, %l0
	...
	!instruction = FSUBq
	!basic case: 5.0 - 3.0 = 2.0
	set A, %l0
	ld [%l0], %f0
	ld [%l0+4], %f1
	ld [%l0+8], %f2
	ld [%l0+12], %f3	! f0-f3 = 5.0 (quad)
	set B, %l1
	ld [%l1], %f4
	ld [%l1+4], %f5
	ld [%l1+8], %f6
	ld [%l1+12], %f7	! f4-f7 = 3.0 (quad)
	fsubq %f0, %f4, %f8	! f8-f11 = 2.0 (quad)
	rd %psr, %o1		! Expected o1=0x10E0

	!invalid case: (+inf) - (+inf) -- traps, g2 -> 1
	set C, %l2
	ld [%l2], %f12
	ld [%l2+4], %f13
	ld [%l2+8], %f14
	ld [%l2+12], %f15	! f12-f15 = +inf (quad)
	fsubq %f12, %f12, %f16	! invalid: (+inf)-(+inf)
...
not_reached:
	...
	.align 8
mFSR: .word 0
A: .word 0x40014000, 0x00000000, 0x00000000, 0x00000000
