...
!	or %l1, %l0, %l0
	...
	fcmpq %f0, %f4		! 2.0 < 3.0 -> fcc=1
	set fsrsave0, %o4
	st %fsr, [%o4]
	ld [%o4], %o0		! o0: fsr, low bits carry the fcc field
	rd %psr, %o1		! Expected o1=0x10E0

	!a quiet NaN operand does NOT trap FCMPq (only a signaling NaN would) --
	!fcc should read 3 (unordered), g2 stays 0 (no trap)
	set C, %l2
	ld [%l2], %f12
	ld [%l2+4], %f13
	ld [%l2+8], %f14
	ld [%l2+12], %f15	! f12-f15 = quiet NaN (quad)
	fcmpq %f0, %f12	! 2.0 vs qNaN -> fcc=3 (unordered), no trap
	set fsrsave1, %o4
	st %fsr, [%o4]
	ld [%o4], %o2		! o2: fsr, low bits carry the fcc field
...
C: .word 0x7fff8000, 0x00000000, 0x00000000, 0x00000000
