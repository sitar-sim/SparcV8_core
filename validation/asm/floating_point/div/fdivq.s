! Functional test for this project's quad-precision support.
! (AJIT's own suite assumes quad-precision FP ops are unimplemented; this
! project implements them, so this test -- unlike the rest of this corpus --
! is written for this project rather than adapted from AJIT. See
...
_start:
	...
	nop
	nop
!	mov 0x00001000, %l1	!Enabling FPU
!	or %l1, %l0, %l0
	wr %l0, %psr
	...
	! initialize fpu-fsr so that bit 27 is 1 (NVM=1), so the invalid-operation
	! cases below actually trap instead of just setting cexc/aexc silently.
	mov 0x1, %l1
	mov 0x1b, %l2
	sll %l1, %l2, %l2
	set mFSR, %l3
	...
	!instruction = FDIVq
	!basic case: 6.0 / 3.0 = 2.0
	set A, %l0
	ld [%l0], %f0
	ld [%l0+4], %f1
	ld [%l0+8], %f2
	ld [%l0+12], %f3	! f0-f3 = 6.0 (quad)
	set B, %l1
	ld [%l1], %f4
	ld [%l1+4], %f5
	ld [%l1+8], %f6
	ld [%l1+12], %f7	! f4-f7 = 3.0 (quad)
	fdivq %f0, %f4, %f8	! f8-f11 = 2.0 (quad)
	rd %psr, %o1		! Expected o1=0x10E0

	!invalid case: 0/0 -- traps, g2 -> 1
	set C, %l2
	ld [%l2], %f12
	ld [%l2+4], %f13
	ld [%l2+8], %f14
	ld [%l2+12], %f15	! f12-f15 = 0.0 (quad)
	fdivq %f12, %f12, %f16	! invalid: 0/0
	rd %psr, %o2		! Expected o2=0x10E0

	!division-by-zero case: 6.0/0.0 -- DZM (bit 24) was never enabled above (only
	!NVM was), so this does NOT trap -- it silently completes and records the
	!division-by-zero exception in FSR.cexc (bit 1) instead. g2 stays at 1.
	fdivq %f0, %f12, %f20	! division-by-zero: 6.0/0.0 -- f20-f23 = +inf (quad)
	set mFSR, %o4
	st %fsr, [%o4]
	ld [%o4], %o3		! Expected o3 bit1 (0x2) set: division-by-zero
	rd %psr, %o5		! Expected o5=0x10E0
...
not_reached:
	...
	.align 8
mFSR: .word 0
A: .word 0x40018000, 0x00000000, 0x00000000, 0x00000000
B: .word 0x40008000, 0x00000000, 0x00000000, 0x00000000
