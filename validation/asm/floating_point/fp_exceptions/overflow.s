! Test: FSR fp_exception via IEEE-754 overflow (OFM enabled), distinct
! from floating_point/invalid_ops/ (which only exercises the invalid-
! operation case).
!
! FLT_MAX (0x7F7FFFFF, ~3.4028235e38) * 2.0f overflows single precision
! range -- OFM=1 so this must trap fp_exception/IEEE_754_exception, with
! FSR.cexc <- most_significant_bit_of(texc and TEM) = ofc only (bit 3),
! per Appendix C.7's complete_fp_execution algorithm (masking by TEM
! selects only the overflow bit, even though this operation also raises
...
.section	".data"
.align 8
FLT_MAX: .word 0x7F7FFFFF	! largest finite single-precision value
...
_start:
	...
	! Enable FP unit (PSR.EF, bit 12), and OFM (overflow trap-enable,
	! FSR bit 26 = 0x04000000). All other FSR trap-enables stay 0.
	rd %psr, %l0
	set 0x1000, %l1
	or %l0, %l1, %l0
	wr %l0, %psr
	...
	!load operands
	set FLT_MAX, %l0; ld [%l0], %f0
	set TWO, %l0;     ld [%l0], %f1

	!======================================
	! Excite overflow.
	fmuls %f0, %f1, %f2		! FLT_MAX * 2.0 overflows -- should trap.

	! Execution resumes here after the trap handler's rett. Snapshot FSR
	! immediately -- expect cexc.ofc=1 (bit 3), nothing else has touched
	! FSR in between.
	set fsr_snapshot, %o0
	st %fsr, [%o0]
