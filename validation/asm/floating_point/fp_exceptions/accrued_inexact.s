! Test: FSR accrued-inexact (aexc.nxa) tracking, per the SPARC V8 manual's
! FSR conformance requirement (Chapter 4, "FSR Conformance") -- see
! compliance/README.md Issue 1 for the full writeup and worked example this
! test formalizes. Unlike the AJIT-adapted tests elsewhere in validation/,
! this test asserts our own model's behavior directly against the manual,
! not against another reference implementation's expected values.
!
! FSR layout (Registers.h): cexc = FSR[4:0] (nvc,ofc,ufc,dzc,nxc, bit4..0),
! reflects only the MOST RECENTLY completed FPop. aexc = FSR[9:5]
! (nva,ofa,ufa,dza,nxa, bit9..5), a STICKY OR-accumulator, cleared only by
! software.
!
! Sequence:
!   1. NXM (inexact trap-enable) = 0: an inexact FPop must NOT trap, only
!      set cexc.nxc and accumulate into aexc.nxa, silently.
!   2. fdivs 1.0/3.0 (single-precision) -- genuinely inexact (1/3 has no
!      finite binary representation), and nothing else (no overflow,
!      underflow, or invalid operand involved). After this: cexc.nxc=1,
!      aexc.nxa=1.
!   3. Snapshot FSR to memory (fsr_snapshot1) -- check both bit 0 (nxc)
!      and bit 5 (nxa) are set.
!   4. fmovs 0.0 -> %f2 -- an ordinary FPop that raises no exception at
!      all (texc=0). Per the manual, cexc <- texc on every FPop
!      completion, so this clears cexc back to 0 -- but aexc is only ever
!      OR-accumulated (aexc <- aexc | texc), so aexc.nxa must remain set.
!   5. Snapshot FSR again (fsr_snapshot2) -- check bit 0 (nxc) is now
!      clear, while bit 5 (nxa) is still set. This is the distinguishing
!      behavior between "current" and "accrued" exception state.
!
...
.section	".data"
.align 8
ONE:   .word 0x3F800000	! 1.0f
THREE: .word 0x40400000	! 3.0f
ZERO:  .word 0x00000000	! 0.0f
...
_start:
	...
	! Enable FP unit (PSR.EF, bit 12). All FSR trap-enable bits (TEM,
	! bits 27:23) stay 0 -- nothing in this test should ever trap.
	rd %psr, %l0
	set 0x1000, %l1
	or %l0, %l1, %l0
	wr %l0, %psr
	...
	!load operands
	set ONE, %l0;   ld [%l0], %f0	! f0 = 1.0f
	set THREE, %l0; ld [%l0], %f1	! f1 = 3.0f
	set ZERO, %l0;  ld [%l0], %f3	! f3 = 0.0f (source for the later fmovs)

	!======================================
	! step 2: a genuinely inexact op (1.0/3.0), NXM=0 so it accumulates
	! silently into aexc.nxa instead of trapping.
	fdivs %f0, %f1, %f2		! f2 = 1.0/3.0 (inexact)

	! step 3: snapshot FSR -- expect cexc.nxc=1 (bit0) and aexc.nxa=1 (bit5)
	set fsr_snapshot1, %o0
	st %fsr, [%o0]

	!======================================
	! step 4: an ordinary FPop that raises no exception at all
	fmovs %f3, %f2			! f2 = 0.0f, texc=0

	! step 5: snapshot FSR again -- expect cexc.nxc=0 (bit0 cleared by
	! this FPop), but aexc.nxa still =1 (bit5, sticky)
	set fsr_snapshot2, %o1
	st %fsr, [%o1]
...
not_reached:
	...
	.align 8
fsr_snapshot1: .word 0
fsr_snapshot2: .word 0
