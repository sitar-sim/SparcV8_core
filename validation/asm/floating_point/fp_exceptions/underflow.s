! Test: FSR fp_exception via IEEE-754 underflow (UFM enabled), distinct
! from floating_point/invalid_ops/ (which only exercises the invalid-
! operation case) and fp_exceptions/overflow.s.
!
! FLT_MIN_NORMAL (0x00800000, ~1.1754944e-38, the smallest positive
! single-precision normal) * 0.5f produces a true result in the subnormal
! range -- UFM=1 so this must trap fp_exception/IEEE_754_exception, with
! FSR.cexc <- most_significant_bit_of(texc and TEM) = ufc only (bit 2),
! per Appendix C.7's complete_fp_execution algorithm (masking by TEM
! selects only the underflow bit, even though this operation also raises
! inexact -- NXM is not enabled here, so nx does not survive the mask).
! g2 is incremented in the trap handler and execution resumes at the very
! next instruction (an FSR snapshot store), so FSR still holds this
! trap's cexc value at that point.
!
...
.section	".data"
.align 8
FLT_MIN_NORMAL: .word 0x00800000	! smallest positive single-precision normal
TENTH:          .word 0x3DCCCCCD	! 0.1f -- deliberately not a power of two,
					! so the (otherwise exactly-representable-as-
					! a-subnormal) product is also inexact:
					! 0.5f would land on an exact subnormal
					! value and not actually set UF under the
					! "tininess with loss of accuracy" rule
					! most FPUs (including the host x86 one
					! this model brackets against) use.
...
_start:
	...
	mov 0xBAD, %g1
	mov 0x0, %g2	! incremented by the fp_exception trap handler

	! Enable FP unit (PSR.EF, bit 12), and UFM (underflow trap-enable,
	! FSR bit 25 = 0x02000000). All other FSR trap-enables stay 0.
	rd %psr, %l0
	set 0x1000, %l1
	or %l0, %l1, %l0
	wr %l0, %psr
	...
	!load operands
	set FLT_MIN_NORMAL, %l0; ld [%l0], %f0
	set TENTH, %l0;          ld [%l0], %f1

	!======================================
	! Excite underflow.
	fmuls %f0, %f1, %f2		! FLT_MIN_NORMAL * 0.1 underflows -- should trap.

	! Execution resumes here after the trap handler's rett. Snapshot FSR
	! immediately -- expect cexc.ufc=1 (bit 2), nothing else has touched
	! FSR in between.
	set fsr_snapshot, %o0
	st %fsr, [%o0]
...
HW_trap_0x08: inc %g2; rett %r18; nop; nop !fp_exception trap handler
