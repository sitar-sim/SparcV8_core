! Test: FSR fp_exception via IEEE-754 division-by-zero (DZM enabled) --
! i.e. finite_nonzero/0.0, which per IEEE 754 sets DZ (not NV/invalid --
! that's reserved for 0/0 and inf/inf, already covered by
! floating_point/invalid_ops/'s "fdivs %f2,%f2,%f7" case).
!
! 1.0f / 0.0f -- DZM=1 so this must trap fp_exception/IEEE_754_exception,
! with FSR.cexc <- most_significant_bit_of(texc and TEM) = dzc only
! (bit 1), per Appendix C.7's complete_fp_execution algorithm. Division
! by zero is exact by IEEE 754 definition (the result is +-infinity, not
! a rounded approximation), so unlike overflow/underflow this does not
! also raise inexact.
...
_start:
	...
	! Enable FP unit (PSR.EF, bit 12), and DZM (division-by-zero
	! trap-enable, FSR bit 24 = 0x01000000). All other FSR trap-enables
	! stay 0.
	rd %psr, %l0
	set 0x1000, %l1
	or %l0, %l1, %l0
	...
	!load operands
	set ONE, %l0;  ld [%l0], %f0
	set ZERO, %l0; ld [%l0], %f1

	!======================================
	! Excite division-by-zero.
	fdivs %f0, %f1, %f2		! 1.0 / 0.0 -- should trap.

	! Execution resumes here after the trap handler's rett. Snapshot FSR
	! immediately -- expect cexc.dzc=1 (bit 1), nothing else has touched
	! FSR in between.
	set fsr_snapshot, %o0
	st %fsr, [%o0]
