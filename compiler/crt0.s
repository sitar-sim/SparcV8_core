! crt0.s
!
! Minimal startup stub for bare-metal C test programs, linked ahead of a
! test's own compiled object by compile_c.sh (entry point overridden to
! _start there, via `sparc-elf-ld -e _start`).
!
! Does the same setup every validation/asm/ test does by hand (see "Anatomy
! of a test" in writing_and_running_assembly_programs.md): enables traps,
! points TBR at a full 256-entry trap table, and initializes %g1 as the
! pass/fail sentinel, so a C test's `ta 0` halts exactly the same way an
! assembly test's does, and an unexpected trap is caught the same way too.
!
! It additionally initializes %sp (and, through it, %fp), which the
! assembly tests never need to do by hand (none of them call a function,
! so none of them ever execute a `save`). Every register, including %sp,
! is 0 after reset. Compiled C code executes `save %sp, -N, %sp` at the
! start of any function that needs one, which is any function using a
! local variable, even at -O0. Without this, %sp stays 0 and every local
! variable access silently falls outside MemCore's bounds.
!
! %sp is set up AFTER the PSR write below, not before. PSR's low 5 bits
! are CWP (current window pointer), and the boot-time reset trap has
! already left CWP pointing at a window other than 0. Writing PSR (with
! a CWP field of 0) switches the window back to 0, which would silently
! hide any register value (including %sp) set beforehand in the old
! window. Every validation/asm/ test structures things the same way,
! doing all of its own work only after this same PSR/TBR setup block.
...
_start:
	...
	! Enable traps: set PSR with ET=1, PS=1, S=1, all other fields=0
	mov 0xE0, %l0
	wr %l0, %psr
	...
	! Initialize the stack. STACK_TOP is comfortably inside the model's
	! 256MB memory (see MemCore.h), with plenty of headroom below it for
	! the stack to grow down into.
	set 0x0ffffff0, %sp
	...
	!======================================
	! control should NOT reach here: main() must never return (see
	! writing_and_running_c_programs.md)
not_reached:
	set 0xDEAD, %g1
	ta 0
	nop
